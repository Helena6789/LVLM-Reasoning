...
class ConversationsSubimagePreprocessor(MediaMixin, RowPreprocessMixin):
    ...
    def preprocess(self, d: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            row.update({
                ...
                'response': response,
                'tools': tools,
                'subimage_scores': d.get('subimage_scores'),
            })
            medias = self.parse_medias(d)
            self.media_replacer(row, medias)
            # print("row", row)
...
class SmartPreprocessor:
    ...
    def __init__(self) -> None:
        self.preprocessor_mapping = {
            ...
            'chatml_subimage': {
                'required': ['messages'],
                'preprocessor':
                ConversationsSubimagePreprocessor(conversations_key='messages', from_key='role', value_key='content')
            },
            'sharegpt': {
    ...
    def _get_preprocessor(self, dataset: DATASET_TYPE) -> PreprocessFunc:
        ...
        for k, required_keys in required_keys_mapping.items():
            ...
            # print("set(required_keys) - keys", set(required_keys) - keys)
