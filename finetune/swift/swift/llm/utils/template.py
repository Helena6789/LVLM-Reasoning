...
class Template:
    ...
    def training_context(self):
        ...
        def _pre_forward_hook(module, args, kwargs):
            ...
            # print("data", data)
            # input()
            # kwargs.update({"subimage_info": "test"})
            return args, kwargs
...
class InternvlTemplate(Template):
    ...
    def _post_encode(self, model, data: Any) -> Dict[str, Any]:
        ...
        if pixel_values is not None:
            ...
            ################### Check if pixel_values correspond to the image correctly ###################
            # IMAGENET_MEAN = (0.485, 0.456, 0.406)
            # IMAGENET_STD = (0.229, 0.224, 0.225)
            # output_dir = "/scratch/czr/LVLM-Reasoning/debug"
            # # Define patch size and grid size
            # patch_size = 14
            # grid_size = 448 // patch_size  # 32 patches along each dimension

            # # Function to de-normalize pixel values
            # def denormalize(image_tensor):
            #     for c in range(3):
            #         image_tensor[c] = image_tensor[c] * IMAGENET_STD[c] + IMAGENET_MEAN[c]
            #     return image_tensor

            # # Loop through the grid, extract, denormalize, and save each patch
            # for i in range(grid_size):
            #     for j in range(grid_size):
            #         # Extract the patch and move it to CPU for processing
            #         patch = pixel_values[0, :, i*patch_size:(i+1)*patch_size, j*patch_size:(j+1)*patch_size].to(dtype=torch.float32).cpu()
                    
            #         # De-normalize and clamp values between 0 and 1
            #         patch = denormalize(patch).clamp(0, 1)
            ...
            #         patch_path = os.path.join(output_dir, f"patch_{i}_{j}.png")
            #         patch_image.save(patch_path)

            # input("all the image saved")
            ################### Check if pixel_values correspond to the image correctly ###################
